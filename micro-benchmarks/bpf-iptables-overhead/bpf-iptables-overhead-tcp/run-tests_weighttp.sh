...
ssh polycube@$REMOTE_DUT << EOF
  set -x
  sudo docker exec -d bpf-iptables bash -c "$REMOTE_FOLDER/rulesets/rules_0.sh $IPTABLES INPUT $LOCAL_CLIENT_ADDR $REMOTE_SERVER_PORT"
EOF
}
...
for run in `seq 1 $NUMBER_RUNS`; do
  ...
  for range_value in "${test_range[@]}"; do
    ...
    set_irq_affinity "all"
    weighttp -n 100000 -c $range_value -t $THREAD_COUNT http://$REMOTE_SERVER_ADDR:$REMOTE_SERVER_PORT/$REMOTE_SERVER_FILE &> /tmp/weighttp_output.txt
