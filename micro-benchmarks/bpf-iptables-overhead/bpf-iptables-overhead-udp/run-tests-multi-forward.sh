...
ssh polycube@$REMOTE_DUT << EOF
  ...
  sudo docker exec -d bpf-iptables bash -c "$REMOTE_FOLDER/rulesets/rules_$size.sh $IPTABLES FORWARD"
EOF
}
...
ssh polycube@$REMOTE_DUT << EOF
  ...
  sudo docker rm -f bpf-iptables &> /dev/null
...
for size in "${ruleset_values[@]}"; do
  ...
  cd $PKTGEN_FOLDER
  sudo ./app/x86_64-native-linuxapp-gcc/pktgen -c ff -n 4 --proc-type auto --file-prefix pg -- -T -P -m "[1:2/3/4/5].0, [6/7].1" -f $DIR/bpf-iptables-overhead.lua
  sleep 5
